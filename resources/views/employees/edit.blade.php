...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
        <h1 class="text-xl font-bold text-slate-900">Edit Employee</h1>
        <p class="text-sm text-slate-500 mt-0.5">{{ $employee->full_name }} — {{ $employee->employee_code }}</p>
    </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="employee_code" name="employee_code" value="{{ old('employee_code', $employee->employee_code) }}" required class="input-text w-full @error('employee_code') input-error @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" id="first_name" name="first_name" value="{{ old('first_name', $employee->first_name) }}" required class="input-text w-full @error('first_name') input-error @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" id="last_name" name="last_name" value="{{ old('last_name', $employee->last_name) }}" required class="input-text w-full @error('last_name') input-error @enderror">
                    ...
                    <div>
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email', $employee->email) }}" required class="input-text w-full @error('email') input-error @enderror">
                        @error('email') <p class="field-error">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="phone" class="block text-sm font-medium text-slate-700 mb-1">Phone</label>
                        <input type="text" id="phone" name="phone" value="{{ old('phone', $employee->phone) }}" class="input-text w-full">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="address" class="block text-sm font-medium text-slate-700 mb-1">Address</label>
                        <textarea id="address" name="address" rows="2" class="input-text w-full">{{ old('address', $employee->address) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="position_id" name="position_id" required class="input-text w-full">
                            ...
                            @foreach($positions as $p)
                                <option value="{{ $p->id }}" {{ old('position_id', $employee->position_id) == $p->id ? 'selected' : '' }}>{{ $p->name }}{{ $p->department ? ' ('.$p->department->name.')' : '' }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" id="hire_date" name="hire_date" value="{{ old('hire_date', $employee->hire_date?->format('Y-m-d')) }}" required class="input-text w-full">
                    ...
                    <div>
                        ...
                        <input type="number" id="salary" name="salary" value="{{ old('salary', $employee->salary) }}" step="0.01" min="0" class="input-text w-full">
                    </div>
                </div>
            </div>
