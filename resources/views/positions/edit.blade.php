...
<div class="mb-6">
    ...
    <h1 class="text-xl font-bold text-slate-900 mt-2">Edit Position</h1>
    <p class="text-sm text-slate-500 mt-0.5">Update position information</p>
</div>
        ...
        <form method="POST" action="{{ route('positions.update', $position) }}">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $position->name) }}" class="input-text mt-1 @error('name') input-error @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="base_salary" id="base_salary" value="{{ old('base_salary', $position->base_salary) }}" step="0.01" min="0" class="input-text mt-1 @error('base_salary') input-error @enderror">
                    @error('base_salary') <p class="field-error">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="description" class="block text-sm font-medium text-slate-700">Description</label>
                    <textarea name="description" id="description" rows="3" class="input-text mt-1 @error('description') input-error @enderror">{{ old('description', $position->description) }}</textarea>
