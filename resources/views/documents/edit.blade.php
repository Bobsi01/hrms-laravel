...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
        <h1 class="text-xl font-bold text-slate-900">Edit Document</h1>
        <p class="text-sm text-slate-500 mt-0.5">Update document details and assignments.</p>
    </div>
</div>
        ...
        <form method="POST" action="{{ route('documents.update', $document) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $document->title) }}" class="input-text w-full @error('title') input-error @enderror" required>
                ...
                <div>
                    ...
                    <select name="doc_type" class="input-text w-full @error('doc_type') input-error @enderror" required>
                        <option value="memo" {{ old('doc_type', $document->doc_type) === 'memo' ? 'selected' : '' }}>Memo</option>
                        <option value="contract" {{ old('doc_type', $document->doc_type) === 'contract' ? 'selected' : '' }}>Contract</option>
                        <option value="policy" {{ old('doc_type', $document->doc_type) === 'policy' ? 'selected' : '' }}>Policy</option>
                        <option value="other" {{ old('doc_type', $document->doc_type) === 'other' ? 'selected' : '' }}>Other</option>
                    </select>
            ...
            <div class="mb-4">
                ...
                <input type="file" name="file" class="input-text w-full @error('file') input-error @enderror" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.jpg,.jpeg,.png,.gif,.webp,.zip">
                <p class="text-xs text-slate-400 mt-1">Leave empty to keep current file. Current: <span class="font-mono">{{ basename($document->file_path) }}</span></p>
            ...
            <div class="mb-4">
                ...
                <div class="flex flex-wrap gap-4" x-data="{ assignType: '{{ old('assign_type', $assignType) }}' }">
                        ...
                        <div class="border border-slate-200 rounded-lg max-h-48 overflow-y-auto p-2 space-y-1">
                            ...
                            <label class="flex items-center gap-2 px-2 py-1 rounded hover:bg-slate-50 cursor-pointer">
                                <input type="checkbox" name="employee_ids[]" value="{{ $emp->id }}" class="accent-indigo-600"
                                    {{ in_array($emp->id, old('employee_ids', $assignedEmployeeIds)) ? 'checked' : '' }}>
                        ...
                        <div class="border border-slate-200 rounded-lg max-h-48 overflow-y-auto p-2 space-y-1">
                            ...
                            <label class="flex items-center gap-2 px-2 py-1 rounded hover:bg-slate-50 cursor-pointer">
                                <input type="checkbox" name="department_ids[]" value="{{ $dept->id }}" class="accent-indigo-600"
                                    {{ in_array($dept->id, old('department_ids', $assignedDepartmentIds)) ? 'checked' : '' }}>
